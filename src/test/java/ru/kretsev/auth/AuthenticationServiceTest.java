...
class AuthenticationServiceTest {
    ...
    void testAuthenticate_Success() {
        ...
        when(authenticationManager.authenticate(any(UsernamePasswordAuthenticationToken.class)))
                .thenReturn(new UsernamePasswordAuthenticationToken(user, PASSWORD)); // Мокаем успешную аутентификацию
    ...
    void testAuthenticate_Failed_WrongPassword() {
        ...
        when(userRepository.findByEmail(EMAIL)).thenReturn(Optional.of(user));
        when(authenticationManager.authenticate(any(UsernamePasswordAuthenticationToken.class)))
                .thenThrow(new RuntimeException("Invalid credentials")); // Мокаем исключение
